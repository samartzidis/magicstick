...
namespace HidLibrary
{
    public class HidDevice : IHidDevice
    {
        ...
        private void OpenDevice(DeviceMode readMode, DeviceMode writeMode, ShareMode shareMode)
        {
            if (IsOpen) 
                return;
            ...
            try
            {
                ReadHandle = OpenDeviceIO(_devicePath, DeviceMode.Overlapped, NativeMethods.GENERIC_READ, shareMode);
                WriteHandle = OpenDeviceIO(_devicePath, DeviceMode.Overlapped, NativeMethods.GENERIC_WRITE, shareMode);
            }
            catch (Exception exception)
            {
                IsOpen = false;
        ...
        public HidDeviceData Read(int timeout = 0)
        {
            if (IsConnected)
            {
                if (IsOpen == false) OpenDevice(DeviceMode.Overlapped, DeviceMode.Overlapped, _deviceShareMode);
                try
                {
        ...
        /// 
        public HidReport GetInputReport(byte reportId)
        {
        ...
        public bool Write(byte[] data, int timeout = 0)
        {
            if (IsConnected)
            {
                if (IsOpen == false) OpenDevice(DeviceMode.Overlapped, DeviceMode.Overlapped, _deviceShareMode);
        ...
        public bool WriteReport(HidReport report, int timeout = 0)
        {
        ...
        /// 
        public bool SetOutputReport(HidReport report)
        {
        ...
        private bool WriteData(byte[] data, int timeout)
        {
            ...
            try
            {
                bool success = NativeMethods.WriteFile(WriteHandle, buffer, (uint)buffer.Length, out bytesWritten, ref overlapped);
                uint lastError = NativeMethods.GetLastError();

                if (success)
                {
                    // Synchronous completion - still need to get the result
                    bool result = NativeMethods.GetOverlappedResult(WriteHandle, ref overlapped, out bytesWritten, false);
                    return result;
                }
                else if (lastError == NativeMethods.ERROR_IO_PENDING)
                {
                    ...
                    switch (result)
                    {
                        case NativeMethods.WAIT_OBJECT_0:
                            // Operation completed
                            bool overlappedResult = NativeMethods.GetOverlappedResult(WriteHandle, ref overlapped, out bytesWritten, false);
                ...
                {
                    // Device disconnection or other error - handle is likely invalid
                    // Close the device to force reconnection on next operation
                    CloseDevice();
                    return false;
                }
            }
        ...
        protected HidDeviceData ReadData(int timeout)
        {
            ...
            if (_deviceCapabilities.InputReportByteLength > 0)
            {
                ...
                try
                {
                    ...
                    {
                        // Synchronous completion - still need to get the result
                        bool result = NativeMethods.GetOverlappedResult(ReadHandle, ref overlapped, out bytesRead, false);
                    ...
                    else if (lastError == NativeMethods.ERROR_IO_PENDING)
                    {
                        ...
                        switch (result) 
                        {
                            case NativeMethods.WAIT_OBJECT_0:
                                // Operation completed
                                bool overlappedResult = NativeMethods.GetOverlappedResult(ReadHandle, ref overlapped, out bytesRead, false);
                                status = overlappedResult ? HidDeviceData.ReadStatus.Success : HidDeviceData.ReadStatus.ReadError;
                                break;
                            case NativeMethods.WAIT_TIMEOUT:
                                // Cancel the pending overlapped I/O operation to prevent interference with subsequent reads
                                NativeMethods.CancelIoEx(ReadHandle, ref overlapped);
                    ...
                    // Only copy buffer if we have valid data
                    if (status == HidDeviceData.ReadStatus.Success && bytesRead > 0)
                    {
        ...
        {
            if (!IsOpen) OpenDevice(DeviceMode.Overlapped, DeviceMode.Overlapped, _deviceShareMode);
            Inserted?.Invoke();
        }
