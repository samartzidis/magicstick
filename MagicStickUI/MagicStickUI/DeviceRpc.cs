...
public class DeviceRpc : IDisposable
{
    ...
    public DeviceRpc(ILoggerFactory loggerFactory, HidDevice rpcDeviceEndpoint)
    {
        _rpcDeviceEndpoint = rpcDeviceEndpoint;
        _logger = loggerFactory.CreateLogger<DeviceRpc>();            
    }
    ...
    private async void Read(CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    if (rep.ReportId == Constants.ReportIdHidIo)
                    {
    ...
    public bool SendRpcData(byte[] data)
    {
        ...
        for (var i = 0; i < numChunks; ++i)
        {
            ...
            var rep = new HidReport(chunk.Length) { ReportId = Constants.ReportIdHidIo, Data = chunk };
