...
type MagicStickRPC struct {
	...
	// battery callback for Linux composite devices
	batteryCallback func(raw []byte)
}

func NewMagicStickRPC(dev *usbhid.Device) (*MagicStickRPC, error) {
...
func (r *MagicStickRPC) SetEventCallback(cb func(name string, payload string)) { r.onEvent = cb }

func (r *MagicStickRPC) SetBatteryCallback(cb func(raw []byte)) { r.batteryCallback = cb }

func (r *MagicStickRPC) Start() {
	r.wg.Add(1)
	go func() {
		defer r.wg.Done()
		slog.Debug("RPC goroutine started")
		for {
			...
			go func() {
				rid, buf, err := r.dev.GetInputReport()
				resultCh <- struct {
					rid byte
					buf []byte
					err error
			...
			select {
			case <-r.stopCh:
				slog.Debug("RPC goroutine stop signal received")
				return
			case result := <-resultCh:
				if result.err != nil {
					slog.Error("RPC goroutine device disconnected", "error", result.err)
					return
				}
				if len(result.buf) == 0 {
					continue
				}
				slog.Debug("RPC received report", "rid", fmt.Sprintf("0x%02x", result.rid), "length", len(result.buf), "data", result.buf)

				// Handle battery reports for Linux composite devices
				if result.rid == ReportIdCharger {
					slog.Debug("Battery report received in RPC goroutine", "rid", fmt.Sprintf("0x%02x", result.rid), "length", len(result.buf), "data", result.buf)
					if len(result.buf) >= 2 {
						// Call battery callback if set
						if r.batteryCallback != nil {
							slog.Debug("Calling battery callback from RPC goroutine", "data", result.buf)
							r.batteryCallback(result.buf)
						} else {
							slog.Debug("Battery callback is nil in RPC goroutine")
						}
					}
					continue
				}

				if result.rid != ReportIdHidIo {
					continue
				}
				...
				if len(result.buf) <= dataStart {
					continue
				}

				hdr := result.buf[dataStart]
				length := int(hdr & 0x3f)
				more := (hdr & 0x40) != 0
				if length > len(result.buf)-dataStart-1 {
					length = len(result.buf) - dataStart - 1
				}
				chunk := result.buf[dataStart+1 : dataStart+1+length]
				r.recvMu.Lock()
...
func (r *MagicStickRPC) Stop() {
	slog.Debug("RPC Stop: closing stop channel")
	close(r.stopCh)
	slog.Debug("RPC Stop: waiting for goroutine to finish")
	r.wg.Wait()
	slog.Debug("RPC Stop: goroutine finished")
}

func (r *MagicStickRPC) handlePayload(data []byte) {
...
func (r *MagicStickRPC) sendJSON(b []byte) error {
...
func (r *MagicStickRPC) RpcCall(req any, id string, timeout time.Duration) (string, error) {
	if id == "" {
...
func (r *MagicStickRPC) GetSettings(timeout time.Duration) (*GetSettingsReply, error) {
	id := generateUUID()
...
func (r *MagicStickRPC) SetSettings(req SetSettingsRequest, timeout time.Duration) error {
	if req.Id == "" {
		req.Id = generateUUID()
	}
...
func (r *MagicStickRPC) SaveConfig(timeout time.Duration) error {
	id := generateUUID()
...
func (r *MagicStickRPC) GetKeymap(defaults bool, timeout time.Duration) (*GetKeymapReply, error) {
	id := generateUUID()
...
func (r *MagicStickRPC) SetKeymap(items []string, timeout time.Duration) (*SetKeymapReply, error) {
	id := generateUUID()
