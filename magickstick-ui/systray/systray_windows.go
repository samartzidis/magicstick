...
const (
	...
	WM_TASKBARCREATED = 0x0400 + 1 // Custom message for taskbar creation
...
type windowsTray struct {
	...
	// Explorer monitoring
	explorerMonitorRunning bool
	explorerMonitorCancel  context.CancelFunc
}
...
func runPlatformSpecific(tray *SystemTray, onReady func(), onExit func()) {
	...
	// Create window class
	className, _ := windows.UTF16PtrFromString("WailsTrayWindow")
	wndClass := WNDCLASS{
		Style:         0,
...
func refreshTrayIcon(tray *windowsTray) {
	...
	if tray.tray.tooltip != "" {
		tooltipUTF16, _ := windows.UTF16FromString(tray.tray.tooltip)
...
func (tray *SystemTray) setTooltipWindows(tooltip string) {
	...
	tooltipUTF16, _ := windows.UTF16FromString(tooltip)
...
func (s *SystemTray) quitWindows() {
	...
	// Stop Explorer monitoring
	if wt.explorerMonitorRunning && wt.explorerMonitorCancel != nil {
		wt.explorerMonitorCancel()
		wt.explorerMonitorRunning = false
	}
...
// startExplorerMonitoring starts monitoring for Explorer process restart
func (wt *windowsTray) startExplorerMonitoring() {
	if wt.explorerMonitorRunning {
		return
	}

	ctx, cancel := context.WithCancel(wt.ctx)
	wt.explorerMonitorCancel = cancel
	wt.explorerMonitorRunning = true
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				currentPID := wt.getExplorerPID()

				if currentPID == 0 {
					// Explorer is not running
					if lastExplorerPID != 0 {
						slog.Info("Explorer process terminated")
						explorerRestarted = true
					}
				} else if lastExplorerPID != 0 && currentPID != lastExplorerPID {
					// Explorer PID changed (restarted)
					slog.Info("Explorer process restarted", "old_pid", lastExplorerPID, "new_pid", currentPID)
					explorerRestarted = true
				}

				if explorerRestarted && currentPID != 0 {
					// Explorer has restarted, refresh the tray icon
					slog.Info("Refreshing tray icon after Explorer restart")
					time.Sleep(1 * time.Second) // Give Explorer time to fully initialize
					wt.refreshTrayIconAfterExplorerRestart()
					explorerRestarted = false
				}

				lastExplorerPID = currentPID
			}
		}
	}()
}

// getExplorerPID gets the process ID of the Explorer process
func (wt *windowsTray) getExplorerPID() uint32 {
	// Find the Shell_TrayWnd window (main taskbar window)
	shellTrayWnd, _, _ := procFindWindow.Call(
		uintptr(unsafe.Pointer(func() *uint16 { s, _ := windows.UTF16PtrFromString("Shell_TrayWnd"); return s }())),
		0,
	)
	...
	procGetWindowThreadProcessId.Call(shellTrayWnd, uintptr(unsafe.Pointer(&processID)))

	return processID
}

// refreshTrayIconAfterExplorerRestart refreshes the tray icon after Explorer restart
func (wt *windowsTray) refreshTrayIconAfterExplorerRestart() {
	if wt == nil {
		return
	}
	...
	// Re-add the icon with all necessary flags
	wt.notifyData.UFlags = NIF_MESSAGE | NIF_ICON | NIF_TIP

	// Ensure the callback message is properly set
	wt.notifyData.UCallbackMessage = WM_TRAYICON

	// Set the icon if it exists
	if wt.icon != 0 {
		wt.notifyData.HIcon = wt.icon
		wt.notifyData.UFlags |= NIF_ICON
	}

	// Set the tooltip if it exists
	if wt.tray.tooltip != "" {
		tooltipUTF16, _ := windows.UTF16FromString(wt.tray.tooltip)
		copy(wt.notifyData.SzTip[:], tooltipUTF16)
		if len(tooltipUTF16) > 127 {
			wt.notifyData.SzTip[127] = 0
		}
	...
	// Re-add the icon
	ret, _, _ := procShell_NotifyIcon.Call(NIM_ADD, uintptr(unsafe.Pointer(&wt.notifyData)))

	if ret != 0 {
		slog.Info("Tray icon refreshed successfully after Explorer restart")
	} else {
		slog.Error("Failed to re-add tray icon after Explorer restart")
	}
}
