...
export function KeymapPage({ selectedDevice, isDeviceOpened }: KeymapPageProps) {
    const [keymap, setKeymap] = useState<main.GetKeymapReply | null>(null);
    ...
    const loadKeymap = async (defaults: boolean = false) => {
        ...
        try {
            ...
            const deviceKeymap = await MagicStickDevice.GetKeymap(defaults);
    ...
    const saveKeymap = async () => {
        ...
        try {
            ...
            const result = await MagicStickDevice.SetKeymap(lines);
