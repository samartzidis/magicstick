...
interface SettingsPageProps {
    selectedDevice: main.MagicStickInfo | null;
    isDeviceOpened: boolean;
}

export function SettingsPage({ selectedDevice, isDeviceOpened }: SettingsPageProps) {
    const [settings, setSettings] = useState<main.GetSettingsReply | null>(null);
    ...
    const loadSettings = async () => {
        ...
        if (isLoadingRef.current) {
            console.log('[SettingsPage] loadSettings already in progress, skipping');
            return;
        }
        ...
        setInfo(null);
        try {
            console.log('Loading settings for device:', selectedDevice.Serial);

            // Add a small delay to ensure RPC connection is ready
            await new Promise(resolve => setTimeout(resolve, 1000));

            const deviceSettings = await MagicStickDevice.GetSettings();
            setSettings(deviceSettings);
            setError(null); // Clear any previous errors on successful load
            setInfo(null); // Clear any previous info messages on successful load
            console.log('Settings loaded:', deviceSettings);
        } catch (error) {
    ...
    const saveSettings = async () => {
        ...
        setError(null);
        try {
            console.log('Saving settings for device:', selectedDevice.Serial, settings);

            // Create SetSettingsRequest from current settings
            const request = new main.SetSettingsRequest({
            ...
            await MagicStickDevice.SetSettings(request);
    ...
    const handleSettingChange = (key: keyof main.GetSettingsReply, value: boolean | number) => {
        if (!settings) return;
    ...
    useEffect(() => {
        console.log('[SettingsPage] useEffect triggered - isDeviceOpened:', isDeviceOpened, 'selectedDevice:', selectedDevice?.Serial, 'isLoadingRef:', isLoadingRef.current, 'settings:', !!settings);
