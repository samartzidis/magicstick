...
interface GetKeymapReply {
    id: string;
    items: string[];
}

interface SetKeymapReply {
    id: string;
    success: boolean;
    error: string;
}
...
export function KeymapPage({ selectedDevice, isDeviceOpened }: KeymapPageProps) {
    const [keymap, setKeymap] = useState<GetKeymapReply | null>(null);
    ...
    const loadKeymap = async (defaults: boolean = false) => {
        ...
        if (isLoadingRef.current) {
            Log.Debug('[KeymapPage] loadKeymap already in progress, skipping');
            return;
        }

        Log.Debug('[KeymapPage] loadKeymap called, defaults:', defaults);
        isLoadingRef.current = true;
        setIsLoading(true);
        setError(null);
        setInfo(null);
        try {
            Log.Debug('Loading keymap for device:', selectedDevice.Serial, 'defaults:', defaults);

            // Add a small delay to ensure RPC connection is ready
            await new Promise(resolve => setTimeout(resolve, 1000));

            const deviceKeymap = await DeviceService.GetKeymap(defaults);
            if (deviceKeymap) {
            ...
            setError(null);
            setInfo(null);
            Log.Debug('Keymap loaded:', deviceKeymap);
        } catch (error) {
            Log.Error('Failed to load keymap:', error);
    ...
    const saveKeymap = async () => {
        ...
        setError(null);
        try {
            Log.Debug('Saving keymap for device:', selectedDevice.Serial);

            // Convert editor content to keymap items
            const lines = editorContent.split('\n').filter(line => line.trim() !== '');
            const result = await DeviceService.SetKeymap(lines);
            Log.Debug('Keymap save result:', result);
            ...
            Log.Error('Failed to apply keymap:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
    ...
    useEffect(() => {
        Log.Debug('[KeymapPage] useEffect triggered - isDeviceOpened:', isDeviceOpened, 'selectedDevice:', selectedDevice?.Serial, 'isLoadingRef:', isLoadingRef.current, 'keymap:', !!keymap);
